...
const listOfTeams = [
  ...
  },
  {
    ...
    members: ['K Naga Sarachhandra ', 'Shashank vishwas damle ', 'Asmit Sou', 'Aditya Kondam'],
  },
  {
    name: 'Events',
    description:
...
const TeamComponent = ({ team, setActive, active }, ref) => {
...
const Team = motion(React.forwardRef(TeamComponent));

const TeamInfoComponent = ({ team, members }, ref) => {
  const teamMembers = members.filter((member) => team?.members.includes(member.name));
  return (
    <Box
      ref={ref}
      width={'80%'}
...
const TeamInfo = motion(React.forwardRef(TeamInfoComponent));
...
const Teams = () => {
  ...
  const members = membersList.sort((a, b) => a.priority - b.priority);
